...
# path = 'd:/Python/SB/HW33/airflow_hw/'
...
def pipeline() -> None:
    df = pd.read_csv(f'{project_path}data/train/homework.csv')
    ...
    best_pipe.fit(X, y)
    model_filename = f'{project_path}/data/models/cars_pipe_{datetime.now().strftime("%Y%m%d%H%M")}.pkl'
